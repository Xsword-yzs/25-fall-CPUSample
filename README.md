...
- 验收时间：12月28日验收（暂定）。每组10分钟左右，顺序按照组号从1开始，若小组迟到则自动顺延。地点：信息楼B205（排序靠后的同学自己酌情安排抵达时间，场地大小有限，没法给大伙都安排座位；如果因事无法根据序号来，可以和其他组同学交换或者找我单独验收）

- 需要准备的东西：代表你们组最好成绩的一台服务器用来核验功能分，同步更新的小组`Github`仓库，组员到齐，带好纸质版报告。验收完毕没有github仓库的小组需要单独将代码和电子报告按照“第XX组_计算机系统实验代码.zip”和“第XX组_计算机系统实验报告.pdf”格式打包发给我

- 关于验收分数：

  ```verilog
  15分 报告 (12月28日前完成，以“第XX组_计算机系统实验报告.pdf”的格式上传到小组仓库)
  10分 现场测试
  5分  github仓库记录（根据每组实际情况赋分）
  ```
